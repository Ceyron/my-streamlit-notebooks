...
class LinearAffineOperator(eqx.Module):
    linear_affine_effect: Callable[[Array,], Array]
    shape: tuple

    def __init__(self, linear_affine_effect: Callable[[Array,], Array], input_structure: Array):
        """
    ...
    def solve_jax(self, *, method="gmres", **kwargs):
        if method == "gmres":
            return jax.scipy.sparse.linalg.gmres(self.linear_effect, self.rhs, **kwargs)[0]
        else:
    ...
    def __call__(self, force: Array) -> LinearAffineOperator:
        def helmholtz_effect(u: Array) -> Array:
            u_padded = jnp.concatenate([
                jnp.array([self.left_boundary]),
                u,
                jnp.array([self.right_boundary])
            ])
            u_second_derivative = (u_padded[2:] - 2*u_padded[1:-1] + u_padded[:-2]) / self.dx**2
...
force = gaussian_blob
helmholtz = Helmholtz1D(dx=dof_grid[2] - dof_grid[1], wavenumber=wavenumber, left_boundary=left_boundary, right_boundary=right_boundary)
